...
public interface CategoryRepository extends JpaRepository<Category, Long> {
    ...
    @Query("""
        ...
        WHERE (:isActive IS NULL OR c.isActive = :isActive)
          AND l.code = :locale
          AND (:searchKey IS NULL OR LOWER(t.name) LIKE LOWER(CONCAT('%', CAST(:searchKey AS string), '%'))
        ...
        ORDER BY COALESCE(p.sortOrder, c.sortOrder) ASC,
                 ...
                 c.sortOrder ASC
    """)
    Page<Long> findIdsByLocaleAndIsActiveAndSearchKey(@Param("locale") String locale, @Param("isActive") Boolean isActive, @Param("searchKey") String searchKey, Pageable pageable);
    ...
    """)
    Optional<Category> findByPublicIdWithAllTranslation(@Param("publicId") String publicId);
}